...
class HybridWorkflowEngine(QObject):
    ...
    async def execute_enhanced_workflow(self, user_prompt: str, conversation_context: list = None):
        ...
        try:
            ...
            # Phase 2: Micro-Task Generation (using the dedicated engine)
            self.workflow_progress.emit("micro-tasking", "📋 Breaking project into micro-tasks...")
            project_context_for_tasks = {
                "project_name": tech_spec.get("project_name", "Unknown Project"),
                "project_description": tech_spec.get("project_description", "")
            }
            all_micro_tasks = await self.micro_task_engine.create_smart_tasks(
                tech_spec.get("technical_specs", {}),
                project_context_for_tasks
            )
    ...
    async def _execute_hybrid_file_generation(self, tech_spec: dict, project_dir: Path,
        ...
        for idx, (filename, file_spec) in enumerate(files_to_generate.items(), 1):
            ...
            if file_spec.get("skip_generation", False):
                self.detailed_log_event.emit("HybridEngine", "info", f"⏭️ Skipping {filename} (exists in project)", "3")
                continue
            ...
            try:
                ...
                assembled_code = await self._generate_file_with_micro_tasks(
                    filename, file_spec, tech_spec, generated_files, micro_tasks_for_file
                )
                ...
                self.detailed_log_event.emit("HybridEngine", "error", f"❌ Failed to generate {filename}: {str(e)}",
                                             "3")
    ...
    async def _generate_file_with_micro_tasks(self, filename: str, file_spec: dict, tech_spec: dict,
        ...
        """Generate a file using micro-task orchestration. Micro-tasks are passed in."""
        try:
            if not micro_tasks:
                self.detailed_log_event.emit("HybridEngine", "info",
                                             f"No micro-tasks for {filename}, using traditional generation", "4")
                return await self.coder_service.generate_file_from_spec(
                    filename, file_spec, {"description": tech_spec.get("project_description", "")},
    ...
    async def _execute_micro_tasks_in_parallel(self, micro_tasks: List[SimpleTaskSpec]) -> List[Dict[str, Any]]:
        """Execute micro-tasks in parallel using the coder service."""
        if not micro_tasks:
            return []
    ...
    async def _setup_project_directory(self, tech_spec: dict) -> Path:
        ...
        if self.is_existing_project_loaded and self.original_project_path:
            ...
            project_dir = self.project_builder.setup_project_directory(
                project_name=project_name, is_modification=True,
                original_user_prompt=self.original_user_prompt,
                original_project_path=self.original_project_path
            )
        else:
            self.detailed_log_event.emit("HybridEngine", "info", f"Creating new project directory: {project_name}",
                                         "1")
            project_dir = self.project_builder.setup_project_directory(
                project_name=project_name, is_modification=False,
                original_user_prompt=self.original_user_prompt, original_project_path=None
            )
        return project_dir
    ...
    async def _finalize_hybrid_project(self, tech_spec: dict, project_dir: Path, start_time: datetime):
        ...
        try:
            ...
            result = {
                "success": True, "project_directory": str(project_dir),
                "project_name": tech_spec.get("project_name", "hybrid_project"),
                "files_created": [f.name for f in project_dir.rglob("*.py")],
                "execution_time": elapsed_time, "approach": "hybrid_micro_task",
                "num_files": len(list(project_dir.rglob("*.py")))
            }
            self.detailed_log_event.emit("HybridEngine", "success",
    ...
    def _create_basic_tech_spec_from_files(self, project_path: Path) -> dict:
        ...
        return {"project_name": project_path.name, "project_description": f"Existing project with {len(py_files)} Python files",
